...
/* ════════════════════════════════════════════════════════
   ...
   localStorage key: nd_tour_<userId>
════════════════════════════════════════════════════════ */
const getTourKey_ND = (uid) => `nd_tour_${uid || 'default'}`;

const MOTIVOS = [
  { value: 'interes_mora',       label: 'Intereses por mora en el pago' },
  { value: 'ajuste_precio',      label: 'Ajuste de precio (se cobró de menos)' },
  { value: 'gastos_adicionales', label: 'Gastos adicionales (flete, seguro, etc.)' },
  { value: 'error_facturacion',  label: 'Error en facturación (valor menor al real)' },
...
const CONCEPTO_VACIO = { descripcion: '', valor: '', porcentaje_iva: 15 };
...
══════════════════════════════════════════════════════════ */
const TourBienvenida_ND = ({ onCerrar }) => {
  const pasos = [
    { emoji: '📋', titulo: '¿Qué es una Nota de Débito?', texto: 'Documento para cobrar cargos adicionales a un cliente después de la factura: intereses por mora, ajustes de precio o gastos adicionales.' },
    { emoji: '🔍', titulo: 'Paso 1 — Seleccionar Factura', texto: 'Busca la factura finalizada a la que agregarás el cargo. La nota siempre referencia una factura en estado Finalizada.' },
    { emoji: '💰', titulo: 'Paso 2 — Conceptos de Cargo', texto: 'Agrega conceptos con descripción, valor e IVA. Cada concepto es un cargo adicional al cliente.' },
    { emoji: '✅', titulo: 'Emitir la Nota de Débito', texto: 'Al emitir, el cargo queda registrado y aparece en el ATS como venta adicional del período.' },
    { emoji: '🏫', titulo: 'Modo Educativo', texto: 'La Nota de Débito aumenta la obligación del cliente. A diferencia de la Nota de Crédito, se usa para cobrar más, no para reembolsar.' },
  ];
      ...
      <div style={{
        ...
        <div style={{ background: 'linear-gradient(135deg,#0f1f4b,#15389a)', padding: '1.3rem 1.5rem', position: 'relative' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '0.35rem' }}>
            <span style={{ background: 'rgba(255,255,255,0.2)', borderRadius: '99px', padding: '0.2rem 0.65rem', fontSize: '0.67rem', fontWeight: '800', color: 'white', letterSpacing: '0.5px' }}>NOTA DE DÉBITO</span>
          ...
          <p style={{ margin: 0, fontSize: '1.1rem', fontWeight: '900', color: 'white', paddingRight: '2rem' }}>Nota de Débito</p>
...
/* ══════════════════════════════════════════════════════════
   BANNER POST-TOUR
══════════════════════════════════════════════════════════ */
const BannerEdu_ND = ({ onClose, onVerTutorial }) => (
  <div style={{
...
══════════════════════════════════════════════════════════ */
const BarraModoEdu_ND = ({ onVerTutorial }) => (
  <div style={{
...
const NotaDebito = ({ onVolver }) => {
  ...
  const tourKey = React.useMemo(() => {
    try {
      ...
      return getTourKey_ND(u?.id || u?.email || 'default');
    } catch {
      return getTourKey_ND('default');
    }
  }, []);
  ...
  const [motivo,        setMotivo]        = useState('interes_mora');
  ...
  const agregarConcepto  = () => setConceptos(prev => [...prev, { ...CONCEPTO_VACIO }]);
  const eliminarConcepto = (i) => setConceptos(prev => prev.filter((_, idx) => idx !== i));
  const cambiarConcepto  = (i, campo, val) =>
    setConceptos(prev => prev.map((c, idx) => idx === i ? { ...c, [campo]: val } : c));

  const conceptosValidos = conceptos.filter(c => c.descripcion.trim() && parseFloat(c.valor) > 0);
  const subtotal0   = conceptosValidos.filter(c => c.porcentaje_iva == 0).reduce((s, c) => s + parseFloat(c.valor || 0), 0);
  const subtotalIva = conceptosValidos.filter(c => c.porcentaje_iva > 0).reduce((s, c) => s + parseFloat(c.valor || 0), 0);
  const iva         = conceptosValidos.filter(c => c.porcentaje_iva > 0).reduce((s, c) => s + parseFloat(c.valor || 0) * (c.porcentaje_iva / 100), 0);
  const total       = subtotal0 + subtotalIva + iva;

  const emitir = async () => {
    setError('');
    if (conceptosValidos.length === 0) {
      setError('Agrega al menos un concepto con descripción y valor.');
      return;
    }
    setGuardando(true);
    try {
      const payload = {
        ...
        conceptos: conceptosValidos.map(c => ({
          ...
          porcentaje_iva: parseFloat(c.porcentaje_iva),
        })),
      };
      const res = await api.post('/notas-debito/', payload);
      setResultado(res.data);
      setPaso(3);
    } catch (e) {
      setError(e?.response?.data?.detail || 'Error al emitir la nota de débito.');
    } finally {
      setGuardando(false);
    }
  };
  ...
  const BarraContexto = () => (
    <div style={{ background: 'white', borderRadius: '16px', padding: '0.9rem 1.4rem', boxShadow: '0 2px 12px rgba(0,0,0,0.05)', marginBottom: '1.4rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', flexWrap: 'wrap' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
        <div style={{ width: '38px', height: '38px', borderRadius: '10px', background: `linear-gradient(135deg,${NARANJA.primary},${NARANJA.mid})`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="9" y1="15" x2="15" y2="15"/><line x1="12" y1="12" x2="12" y2="18"/></svg>
        </div>
        <div>
          ...
          <p style={{ margin: 0, fontSize: '0.92rem', fontWeight: '900', color: '#0f172a' }}>Nota de Débito</p>
        </div>
      </div>
        ...
        {PASOS.map((s, i) => (
          <React.Fragment key={s.n}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
              <div style={{ width: '24px', height: '24px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.7rem', fontWeight: '800', background: paso >= s.n ? `linear-gradient(135deg,${NARANJA.primary},${NARANJA.mid})` : '#e2e8f0', color: paso >= s.n ? 'white' : '#94a3b8', flexShrink: 0 }}>
                {paso > s.n ? '✓' : s.n}
              </div>
              <span style={{ fontSize: '0.74rem', fontWeight: paso === s.n ? '800' : '500', color: paso >= s.n ? '#0f172a' : '#94a3b8', whiteSpace: 'nowrap' }}>{s.label}</span>
            </div>
            {i < 2 && <div style={{ width: '36px', height: '2px', background: paso > s.n ? NARANJA.mid : '#e2e8f0', margin: '0 0.5rem' }} />}
          </React.Fragment>
        ))}
      </div>
    ...
    return (
      <div style={{ padding: '1.4rem 2rem', fontFamily: "'Nunito','Segoe UI',sans-serif" }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '4rem 2rem', textAlign: 'center', gap: '1rem' }}>
          <div style={{ width: '72px', height: '72px', borderRadius: '50%', background: `linear-gradient(135deg,${NARANJA.primary},${NARANJA.mid})`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '0.5rem' }}>
          ...
          <h2 style={{ fontSize: '1.4rem', fontWeight: '900', color: '#0f172a', margin: 0 }}>¡Nota de Débito emitida!</h2>
          <p style={{ fontSize: '0.88rem', color: '#64748b', margin: 0 }}>El documento ha sido registrado correctamente.</p>
          <div style={{ background: NARANJA.lighter, border: `1.5px solid ${NARANJA.border}`, borderRadius: '14px', padding: '1.2rem 2rem', marginTop: '0.5rem' }}>
            <div style={{ fontSize: '0.72rem', fontWeight: '800', color: NARANJA.primary, textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '0.4rem' }}>Número de comprobante</div>
            <div style={{ fontSize: '1.3rem', fontWeight: '900', color: '#0f172a', fontFamily: 'monospace' }}>{resultado.numero_comprobante}</div>
            <div style={{ fontSize: '1.1rem', fontWeight: '700', color: NARANJA.mid, marginTop: '0.4rem' }}>{fmt(resultado.total)}</div>
          </div>
          <div style={{ display: 'flex', gap: '0.75rem', marginTop: '1rem' }}>
            <button onClick={() => { setPaso(1); setFacturaSelec(null); setConceptos([{ ...CONCEPTO_VACIO }]); setResultado(null); setBusqueda(''); buscarFacturas(); }}
              style={{ padding: '0.7rem 1.4rem', background: NARANJA.lighter, border: `1.5px solid ${NARANJA.border}`, borderRadius: '10px', cursor: 'pointer', fontSize: '0.84rem', fontWeight: '700', color: NARANJA.primary, fontFamily: 'inherit' }}>
  ...
  return (
    <div style={{ padding: '1.4rem 2rem', fontFamily: "'Nunito','Segoe UI',sans-serif" }}>
      ...
      <BarraModoEdu_ND onVerTutorial={verTutorial} />
      ...
      {paso === 1 && (
        <div style={{ background: 'white', borderRadius: '16px', border: '1px solid #f1f5f9', boxShadow: '0 2px 12px rgba(0,0,0,0.05)', overflow: 'hidden' }}>
          <div style={{ padding: '1rem 1.4rem', borderBottom: '1px solid #f8fafc', background: '#fafafa', display: 'flex', alignItems: 'center', gap: '0.65rem' }}>
            <div style={{ width: '4px', height: '20px', borderRadius: '2px', background: NARANJA.mid }} />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: '1.2rem', alignItems: 'start' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.1rem' }}>
            ...
            <div style={{ background: AZUL.lighter, border: `1.5px solid ${AZUL.border}`, borderRadius: '14px', padding: '1rem 1.2rem', display: 'flex', gap: '2rem', alignItems: 'center', flexWrap: 'wrap' }}>
              ...
              <button onClick={() => { setPaso(1); setFacturaSelec(null); }}
            ...
            <div style={{ background: 'white', borderRadius: '16px', border: '1px solid #f1f5f9', boxShadow: '0 2px 12px rgba(0,0,0,0.05)', overflow: 'hidden' }}>
              <div style={{ padding: '0.85rem 1.2rem', borderBottom: '1px solid #f8fafc', background: '#fafafa', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                  ...
                  <h3 style={{ margin: 0, fontSize: '0.88rem', fontWeight: '800', color: '#0f172a' }}>Conceptos de cargo adicional</h3>
                </div>
              ...
              <div style={{ padding: '1rem 1.2rem', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                ...
                {conceptos.map((c, i) => (
                  <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 120px 110px 36px', gap: '0.75rem', alignItems: 'center' }}>
                    ...
                    <div style={{ position: 'relative' }}>
                      <span style={{ position: 'absolute', left: '0.65rem', top: '50%', transform: 'translateY(-50%)', fontSize: '0.84rem', color: '#94a3b8', pointerEvents: 'none' }}>$</span>
                      <input type="number" value={c.valor} onChange={e => cambiarConcepto(i, 'valor', e.target.value)}
                        placeholder="0.00" min="0" step="0.01"
                        style={{ ...inputStyle, padding: '0.58rem 0.5rem 0.58rem 1.5rem' }}
                        onFocus={e => e.target.style.borderColor = AZUL.mid}
                        onBlur={e => e.target.style.borderColor = '#e2e8f0'} />
                    </div>
                    <select value={c.porcentaje_iva} onChange={e => cambiarConcepto(i, 'porcentaje_iva', parseFloat(e.target.value))}
                      style={{ ...inputStyle, padding: '0.58rem 0.5rem', cursor: 'pointer' }}>
                      <option value={0}>0%</option>
                      <option value={15}>15%</option>
                    </select>
                    <button onClick={() => eliminarConcepto(i)} disabled={conceptos.length === 1}
                      style={{ width: '32px', height: '32px', borderRadius: '8px', border: '1px solid #fecaca', background: '#fef2f2', cursor: conceptos.length === 1 ? 'not-allowed' : 'pointer', color: '#ef4444', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: conceptos.length === 1 ? 0.3 : 1 }}>
                ...
                <div style={{ padding: '0.6rem 0.8rem', background: AZUL.lighter, borderRadius: '10px', fontSize: '0.76rem', color: AZUL.primary, display: 'flex', gap: '0.4rem', alignItems: 'flex-start', marginTop: '0.25rem' }}>
                  <span>ℹ️</span>
                  <span>La nota de débito <strong>no modifica el stock</strong>. Solo representa un cargo monetario adicional sobre la factura de referencia.</span>
                </div>
              </div>
            </div>
            ...
            <div style={{ background: 'white', borderRadius: '16px', border: '1px solid #f1f5f9', boxShadow: '0 2px 12px rgba(0,0,0,0.05)', padding: '1.2rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
              ...
              <div style={{ gridColumn: 'span 2' }}>
                ...
                <textarea value={observaciones} onChange={e => setObservaciones(e.target.value)}
                  placeholder="Detalla el motivo del cargo adicional..."
          ...
          <div style={{ position: 'sticky', top: '1rem' }}>
            <div style={{ background: 'white', borderRadius: '16px', border: '1px solid #f1f5f9', boxShadow: '0 4px 20px rgba(0,0,0,0.08)', overflow: 'hidden' }}>
              <div style={{ padding: '0.85rem 1.2rem', background: `linear-gradient(135deg,${AZUL.primary},${AZUL.mid})` }}>
                <h3 style={{ margin: 0, fontSize: '0.88rem', fontWeight: '800', color: 'white' }}>Resumen de la ND</h3>
              </div>
              <div style={{ padding: '1.2rem' }}>
                  ...
                  <div style={{ marginBottom: '1rem', display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                    {conceptosValidos.map((c, i) => (
                      <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.78rem', color: '#475569', padding: '0.3rem 0', borderBottom: '1px dashed #f1f5f9' }}>
                        <span style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', paddingRight: '0.5rem' }}>{c.descripcion}</span>
                        <span style={{ fontWeight: '700', flexShrink: 0 }}>{fmt(c.valor)}</span>
                      </div>
                    ))}
                  </div>
                )}
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.55rem', marginBottom: '1rem' }}>
                  ...
                  <ResumenFila label="IVA (15%)" valor={fmt(iva)} />
                  <div style={{ height: '1px', background: '#f1f5f9', margin: '0.25rem 0' }} />
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '0.9rem', fontWeight: '900', color: '#0f172a' }}>TOTAL ND</span>
                ...
                <div style={{ background: '#f8fafc', borderRadius: '10px', padding: '0.75rem', marginBottom: '1rem', fontSize: '0.76rem', color: '#64748b', lineHeight: 1.6 }}>
                  <strong style={{ color: '#0f172a' }}>Al emitir esta nota de débito:</strong>
                  <ul style={{ margin: '0.35rem 0 0', paddingLeft: '1.1rem' }}>
                    <li>El cliente deberá pagar el valor adicional</li>
                    <li>La factura original no se modifica</li>
                    <li>El stock permanece igual</li>
                  </ul>
                </div>
                <button onClick={emitir} disabled={guardando || conceptosValidos.length === 0}
                  style={{ width: '100%', padding: '0.9rem', background: guardando || conceptosValidos.length === 0 ? AZUL.border : `linear-gradient(135deg,${AZUL.primary},${AZUL.mid})`, color: 'white', border: 'none', borderRadius: '12px', cursor: guardando || conceptosValidos.length === 0 ? 'not-allowed' : 'pointer', fontSize: '0.88rem', fontWeight: '800', fontFamily: 'inherit', boxShadow: `0 4px 14px ${AZUL.primary}55` }}>
